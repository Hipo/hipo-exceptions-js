...
class ExceptionTransformer {
    ...
    generateExceptionMap(exception) {
        ...
        if (this.customTransformers && this.customTransformers[exception.type]) {
            ...
            exceptionMap = dataStructureUtils_1.createMapFromObject(exception.detail);
        }
    ...
    */
    generateSpecificFieldError(errorInfo) {
        const errorDetail = errorUtils_1.getErrorDetail(errorInfo);
        try {
            if (errorDetail) {
                return function errorGenerator(fieldName) {
                    return errorUtils_1.generateFieldErrorFromErrorDetail(fieldName, errorDetail);
                };
            }
        }
        catch (error) {
    ...
    generateErrorMessage(errorInfo, options = {}) {
        ...
        try {
            if (!shouldSkipError) {
                const errorDetail = errorUtils_1.getErrorDetail(errorInfo);
                let message = "";
                if (errorDetail && !dataStructureUtils_1.isObjectEmpty(errorDetail)) {
                    message = errorUtils_1.getStringMessage(errorUtils_1.removeKnownKeysFromErrorDetail(errorDetail, knownErrorKeys));
                }
                else {
