...
function generateFieldErrorFromErrorDetail(fieldName, errorDetail) {
    ...
    if (typeof fieldName === "string") {
        ...
        if (errorValue) {
            if (dataStructureUtils_1.isArrayOfStrings(errorValue)) {
...
function getStringMessage(errorDetailValue, key) {
    ...
    if (Array.isArray(errorDetailValue)) {
        if (dataStructureUtils_1.isArrayOfStrings(errorDetailValue)) {
            // errorDetailValue = ["", ""]
            message = generateMessageFromStringArray(errorDetailValue, key);
        }
        else if (dataStructureUtils_1.isArrayOfObjects(errorDetailValue)) {
            // errorDetailValue = [ {}, {}, {..} ]
